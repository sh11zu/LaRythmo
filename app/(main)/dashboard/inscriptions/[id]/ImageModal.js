...
import imageText from '@/data/autorisation_texts.json';
...
export default function ImageModal({ memberName, guardian1, onSign, onClose, readOnly = false, signatureData = null, signedAt = null }) {
  ...
  function handleClose() {
    ...
    if (hasChanges()) { setCloseConfirm(true); } else { onClose(); }
  }
  ...
  }, [authorized, readOnly, closeConfirm]); // eslint-disable-line

  async function handleConfirmSign() {
    setLoading(true);
    await onSign({ authorized });
    setLoading(false);
  }
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-bold text-gray-800">{imageText.IMAGE.title}</h3>
              {readOnly && (
          ...
          <div className="px-6 py-5 overflow-y-auto flex-1 space-y-5 text-sm text-gray-700">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-6">
                {[{ val: true, label: 'J\'autorise' }, { val: false, label: 'Je refuse' }].map(({ val, label }) => (
                  <label
                    key={String(val)}
                    className={`flex items-center gap-2 select-none ${readOnly ? 'cursor-default' : 'cursor-pointer'}`}
                    onClick={() => { if (!readOnly) setAuthorized(val); }}
                  >

// app/(main)/admin/inscriptions/[id]/page.js
// Dossier d'inscription — vue administrateur
...
import DossierDocuments from '@/app/(main)/dashboard/inscriptions/[id]/DossierDocuments';
...
export default async function AdminDossierPage({ params }) {
  ...
  // ── Récupération des données (sans filtre user_id) ──
  const [[insRows], [courseRows], [docRows]] = await Promise.all([
    pool.query(
      `SELECT
         ...
         u.email AS user_email, u.phone AS user_phone, u.address AS user_address, u.gender AS user_gender
       FROM inscriptions i
    ...
    pool.query(
      `SELECT type, status, rejection_reason, file_path, uploaded_at, signed_at, signature_data
       FROM documents
       WHERE member_id = (SELECT member_id FROM inscriptions WHERE id = ?)
  ...
  const acctAddr   = parseAddress(ins.user_address);
  ...
  const guardianName = `${ins.user_first_name} ${ins.user_last_name}`.trim();

  const contacts = {
    guardian1: {
      firstName: ins.user_first_name,
      lastName:  ins.user_last_name,
      gender:    ins.user_gender,
      address:   ins.user_address,
      quality:   ins.guardian1_link_to_member ?? '',
      phone:     ins.user_phone ?? '',
    },
    ...
    <div className="w-full max-w-5xl mx-auto px-4 md:px-8 py-8">
      ...
      <div className="flex items-start justify-between mb-8 gap-4">
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          <span className={`px-3 py-1 rounded-full text-xs font-bold border ${reg.cls}`}>{reg.label}</span>
          <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${pay.cls}`}>{pay.label}</span>
        </div>
      </div>
      ...
      <div className="space-y-5">
        ...
        <AdminActionsCard
          ...
          registrationStatus={ins.registration_status}
          paymentStatus={ins.payment_status}
        />
        ...
        <Section title="Représentants légaux &amp; contact d'urgence">
          <div className="grid grid-cols-2 gap-4 mb-4">
            <CardBlock title="Responsable Légal 1" badge={ins.guardian1_link_to_member}>
              <dl className="grid grid-cols-2 gap-x-4 gap-y-2">
                <InfoRow label="Prénom"    value={ins.user_first_name} />
                <InfoRow label="Nom"       value={ins.user_last_name} />
                <InfoRow label="Email"     value={ins.user_email} />
                <InfoRow label="Téléphone" value={ins.user_phone} />
          ...
          <CardBlock title="Contact d'urgence" badge={ins.emergency_link_to_member}>
        ...
        <Section title="Cours sélectionnés">
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
            <p className="text-sm text-gray-500">{ins.package_id ? 'Forfait' : 'À la carte'}</p>
            <p className="font-mono font-bold text-lg text-gray-800">Total : {price.toFixed(0)} €</p>
          </div>
        </Section>
        ...
        <Section title="Règlement">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-3">
            ...
            <div>
              ...
              <dd className="mt-1">
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${pay.cls}`}>{pay.label}</span>
              </dd>
            </div>
